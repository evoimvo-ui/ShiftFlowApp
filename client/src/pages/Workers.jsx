  ...
  const remove = async id => { 
    if (confirm('Obrisati radnika?')) {
      try {
        ...
        alert('Greška pri brisanju radnika: ' + err.message)
      }
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[--text-primary]">Radnici</h1>
          <p className="text-[--text-muted] text-sm mt-1 font-medium">{workers.length} registrovanih radnika u sistemu</p>
        </div>
        {isAdmin && <Btn onClick={openNew} icon={<Plus size={16} />}>Dodaj radnika</Btn>}
      </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-white/5 border-b border-[--border]">
                {['Ime i prezime', 'Kategorija', 'Maks. sati', 'Kontakt', 'Akcije'].map(h => (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-2 opacity-40">
                      <Users size={48} />
                      <p className="text-sm font-medium">{workers.length === 0 ? 'Nema radnika u bazi.' : 'Nema rezultata za vašu pretragu.'}</p>
                    </div>
                  </td>
                </tr>
              ) : filtered.map((w) => {
