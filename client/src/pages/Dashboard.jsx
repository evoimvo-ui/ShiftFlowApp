...
export default function DashboardPage({ workers, categories, absences, schedules, shiftTypes, settings, refresh, user }) {
  ...
  const handleProcessSwap = async (id, status) => {
    try {
      ...
      const msg = status === 'approved' ? 'Odobreno od strane admina' : status === 'accepted_by_worker' ? 'Prihvaćeno (čeka se admin)' : 'Odbijeno';
      alert(msg)
    } catch (err) {
      alert('Greška pri obradi zahtjeva: ' + err.message)
    }
  }
    ...
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-[--text-primary]">
          {isAdmin ? 'Pregled' : `Zdravo, ${user?.username}! 👋`}
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(() => {
            ...
            return (
              <>
                <StatCard 
                  label="Moji sati ove sedmice" 
                  value={`${hours}h`} 
                  sub={hours > settings.maxHoursPerWeek ? 'Iznad norme' : 'U okviru norme'} 
                  icon={<Clock size={18} />} 
                  color={hours > settings.maxHoursPerWeek ? "#f59e0b" : "#3b82f6"} 
                />
                <StatCard 
                  label="Status" 
                  value={isAbsent ? 'Odsutan' : 'Aktivan'} 
                  sub={isAbsent ? 'Trenutno niste na poslu' : 'Danas radite ili ste slobodni'} 
                  icon={isAbsent ? <UserX size={18} /> : <Zap size={18} />} 
                  color={isAbsent ? "#f43f5e" : "#10b981"} 
                />
                <StatCard 
                  label="Moja kategorija" 
                  value={worker?.categoryIds?.length || 0} 
                  sub="dodijeljenih uloga" 
                  icon={<Tag size={18} />} 
                  color="#8b5cf6" 
                />
              </>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            {isAdmin ? (
              swaps.filter(s => s.status === 'accepted_by_worker').length > 0 && (
                <Badge color="var(--blue)">{swaps.filter(s => s.status === 'accepted_by_worker').length} za odobrenje</Badge>
              )
            ) : (
              swaps.filter(s => s.status === 'pending' && s.targetWorkerId?.name.toLowerCase().includes(user?.username.toLowerCase())).length > 0 && (
                <Badge color="var(--blue)">{swaps.filter(s => s.status === 'pending' && s.targetWorkerId?.name.toLowerCase().includes(user?.username.toLowerCase())).length} novih</Badge>
              )
            )}
          </div>
          ...
          <div className="space-y-3">
            {(() => {
              ...
              if (relevantSwaps.length === 0) {
                return <p className="text-center py-8 text-[--text-muted] italic text-xs">Nema aktivnih zahtjeva za zamjenu.</p>
              }
                ...
                <div key={s.id} className="p-3 bg-white/5 rounded-xl border border-white/5 space-y-3">
                  <div className="flex justify-between items-start">
                    <div className="text-xs">
                      ...
                      <span className="mx-2 text-[--text-muted]">{isAdmin ? 'zamijenio sa' : 'traži zamjenu sa Vama'}</span>
                  ...
                  <div className="flex gap-2">
                    <Btn size="xs" className="flex-1" onClick={() => handleProcessSwap(s.id, isAdmin ? 'approved' : 'accepted_by_worker')} icon={<Check size={12} />}>
                      {isAdmin ? 'Odobri zamjenu' : 'Prihvati zahtjev'}
                    </Btn>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-sm font-bold text-[--text-muted] uppercase tracking-widest flex items-center gap-2">
                ...
                Audit Istorija (Manualne izmjene)
              </h3>
            </div>
            
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {auditLogs.length === 0 && (
                <p className="text-center py-8 text-[--text-muted] italic text-xs">Nema zabilježenih manualnih izmjena.</p>
              )}
              {auditLogs.map(log => {
                ...
                return (
                  <div key={log.id} className="p-3 bg-white/5 rounded-xl border border-white/5 space-y-2">
                    ...
                    <div className="text-xs text-[--text-secondary]">
                      Zamijenio <strong>{oldWorker?.name}</strong> sa <strong>{newWorker?.name}</strong>
                    </div>
                    {log.reason && (
                ...
                <div key={cat.id} className="p-4 bg-[--bg-elevated]/50 rounded-2xl border border-[--border] border-l-4 transition-all hover:translate-y-[-2px] hover:shadow-lg" style={{ borderLeftColor: cat.color }}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-[--text-muted]">Radnika:</span>
