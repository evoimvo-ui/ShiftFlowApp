...
export const Card = ({ children, className = '', style = {} }) => (
  <div className={`bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 ${className}`} style={style}>
    {children}
  </div>
)
...
export const Modal = ({ open, onClose, title, children, width = 500 }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-[var(--bg-elevated)] border border-[var(--border-bright)] rounded-2xl p-7 w-full max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in duration-200" style={{ maxWidth: width }}>
...
export const StatCard = ({ label, value, sub, icon, color = '#3b82f6' }) => (
  <Card className="relative overflow-hidden group">
    ...
    <div className="flex justify-between items-start relative z-10">
      <div>
        <div className="text-[10px] text-[var(--text-muted)] font-bold tracking-widest uppercase mb-2">{label}</div>
        <div className="text-3xl font-extrabold text-[var(--text-primary)] leading-none">{value}</div>
